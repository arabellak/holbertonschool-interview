...
/**
* insert - Function that build an AVL tree from an array
* @array: is a pointer to the first element of the array to be converted
...
avl_t *insert(int *array, size_t start, size_t end, avl_t *parent)
{
	...
	middle = (start + end) / 2;
	root = baby_node(parent, array[middle]);
	if (!root)
		return (NULL);

	if (middle != start)
		root->left = insert(array, start, middle - 1, root);

	if (middle != end)
		root->right = insert(array, middle + 1, end, root);
